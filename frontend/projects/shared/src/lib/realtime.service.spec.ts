...
import { APP_CONFIG, AppConfig } from './platform';
...
function setup(token: string | null) {
  TestBed.configureTestingModule({
    providers: [
      ...
      // The socket origin comes from the platform layer (mobile-app D3):
      // empty `apiBaseUrl` is the same-origin web build, which is what
      // these assertions about `location.host` expect.
      {
        provide: APP_CONFIG,
        useValue: { production: false, apiBaseUrl: '', nativeApiBaseUrl: '' } as AppConfig,
      },
    ],
  });
