...
So the persistence layer becomes IndexedDB storing raw bytes. (`ArrayBuffer` rather than `Blob`, in the end: a `Blob` is the more natural shape — lazily backed, never fully in memory — but it only survives IndexedDB where structured clone knows about it, which is not everywhere this code runs. At a 48 MB ceiling the laziness was not worth an environment dependency.) The queue's *semantics* do not change — FIFO replay, local-id → server-id remap so media queued against an unsynced tower follows it, nothing dropped until the server confirms, failures kept with their reason for retry or discard. Only `load`/`persist` change, which the deviation note in the original tasks.md explicitly named as the swap point. The upload path keeps accepting base64 as well as multipart, so a blob is encoded at send time rather than at queue time.
