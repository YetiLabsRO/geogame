...
- **Backend**: `AdminGameViewSet.destroy` and a new `AdminSessionViewSet.destroy` gain a superuser gate and a live-state gate; a shared `IsSuperUser` permission and a 409 carrying machine-readable blockers, mirroring the 409 shape `IllegalTransition` already uses for the start gate. `UserProfileSerializer` gains `is_superuser` so the SPA can hide a control it would be refused anyway. No migrations — every FK that matters is already `CASCADE` or `SET_NULL`, including `UserProfile.current_session`.
