...
The system SHALL model a `game.Zone` as a geographic polygon that groups towers and awards area-wide points to the team controlling it, where the winning team is determined by the zone's effective conquest rule (see the `scoring` capability).
...
#### Scenario: Per-zone conquest rule override

- **WHEN** an administrator sets the zone's optional `conquest_rule`
- **THEN** its value SHALL be one of `ALL`, `MAJORITY`, or `ANY`, or unset
- **AND** when set it SHALL override the Session and Game conquest-rule configuration for that zone; when unset the zone SHALL inherit the effective rule from the Session or Game (see the `scoring` capability)
...
- **WHEN** an administrator sets the tower's optional `proximity_meters`
- **THEN** it SHALL be a positive integer or unset
- **AND** when set it SHALL be the capture proximity radius for that tower; when unset the tower SHALL fall back to the Game's game-wide `proximity_meters` default (see the `Effective tower proximity radius` requirement)
...
- **AND** proximity filtering SHALL use each tower's effective proximity radius (see the `Effective tower proximity radius` requirement), so towers with their own `proximity_meters` are filtered by that radius rather than the game-wide default

## ADDED Requirements

### Requirement: Effective tower proximity radius

The system SHALL resolve a tower's capture proximity radius as the tower's `proximity_meters` when set, otherwise the Game's game-wide `proximity_meters` default (see the `game-configuration` capability).

#### Scenario: Per-tower override wins

- **WHEN** a tower has a non-null `proximity_meters`
- **THEN** capture verification and proximity filtering for that tower SHALL use the tower's value

#### Scenario: Falling back to the game default

- **WHEN** a tower's `proximity_meters` is unset
- **THEN** the effective radius SHALL be the current Session's Game `proximity_meters` default
- **AND** because every existing tower is unset, migrated games SHALL keep the historical game-wide radius
