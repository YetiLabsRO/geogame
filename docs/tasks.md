...
- [ ] T3.3: Rename `UserProfile.current_game` → `current_session`; replace `/api/current-game/` with `/api/current-session/` (returns session + nested game); remove transitional `Game.start_time` / `Game.end_time`. (Plan: P3.3, Req: 18.2)
