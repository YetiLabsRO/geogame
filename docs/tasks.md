...
- [x] T2C.4: Per-TeamGroup score map view at `/map/<slug>/` (dynamic on TeamGroup.slug). (Plan: P2C.4, Req: 15.1)
- [x] T2C.5: Rules page. (Plan: P2C.5)
...
- [x] T3.2: Add `organize.Session` model (`game`, `slug`, `name`, `start_time`, `end_time`, `is_active`, `created_by`, `created_at`); data migration creates a default Session per existing Game inheriting its dates/flags, re-parents every `Team` off `Game` onto `Session`, drops `Team.game`. `Game.start_time` / `Game.end_time` stay as transitional metadata until T3.3. (Plan: P3.2, Req: 18.1)
- [x] T3.3: Rename `UserProfile.current_game` → `current_session`; replace `/api/current-game/` with `/api/current-session/` (returns session + nested game); remove transitional `Game.start_time` / `Game.end_time`. (Plan: P3.3, Req: 18.2)
